...
let currentJson: ElementJson | undefined = undefined;
      ...
      case "update":
        ...
        if (uijson && !isEqual(uijson, currentJson)) {
          ...
          if (uiContent) {
            ...
            const { setRootContainer, setSelectedComponent } =
            ...
            if (previousSelectedId) {
              const findComponentById = (
                ...
                if (obj instanceof Component) {
                  const compId =
                    obj.userData?.id || (obj.properties?.value as any)?.id;
                  if (compId === targetId) {
                    return obj as Component<any>;
                  }
                }
              ...
              const foundComponent = findComponentById(
                uiContent,
                previousSelectedId
              );
              if (foundComponent) {
            ...
            uiAnchor.add(
              uiRoot.add(
                createInspectorContainer(uiContent, (uid) => {
                  vscode.postMessage({ command: "jump-to", uid });
                  // Find the component by uid and set it as selected
                  const findComponentByUid = (
                    obj: Object3D,
                    targetUid: string
                  ): Component<any> | null => {
                    if (
                      obj.userData &&
                      obj.userData.uid === targetUid &&
                      obj instanceof Component
                    ) {
                      return obj as Component<any>;
                    }
                    for (const child of obj.children) {
                      const found = findComponentByUid(child, targetUid);
                      if (found) {
                        return found;
                      }
                    }
                  ...
                  const foundComponent = findComponentByUid(uiContent, uid);
                  if (foundComponent) {
